...
def add_shufflers(amount,resource_table,table = ['Spy']):
    if amount > len(table) + len(resource_table):
        raise ValueError("Amount requested is higher than total amount of roles.")
    suffle(resource_table)
    for i in range(amount - len(table)):
        table.append(resource_table[i])
    shuffle(table)
    return table

def random_roles(amount,table = ['Spy']):
    
    if name == 'airplane':
        roles = ['first class passenger','air marshall','mechanic','air hostess','copilot','captain','economy class passenger']
        return name, add_shufflers(amount,roles,table)
    if name == 'bank':
